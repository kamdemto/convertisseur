...
class nuts {
    ...
    function __construct($nb, $unit){
      // Nettoyages.
      $this->nb = str_replace(' ', '', $nb);
      //$this->nb = str_replace(',', '', $nb);
 	
      $this->nb = preg_replace('~\.0+$~','',$nb);					// Suppession de tous les espaces.
      ...
      $this->decSep = substr($this->decSep, -2);					// et on prend le dernier des caractères restants.
